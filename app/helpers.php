...
if (!function_exists('wrap_bold')) {
     ...
     * @return $wrap
     */
    function wrap_bold($string, $word = NULL, $tag = 'strong')
    {
        if($word != NULL)
        {
            //$strings = split('/'.$word, $string);
            $wrap = str_ireplace($word, '<'.$tag.' style="color:#333">'.$word.'</'.$tag.'>', $string);
            return $wrap;
        }
        return $string;
    }
